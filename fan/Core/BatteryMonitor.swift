...
class BatteryMonitor: ObservableObject {
    ...
    func updateBatteryInfo() {
        // Do all IOKit work on a background thread to avoid blocking the main thread
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            for ps in sources {
                ...
                // getDetailedBatteryInfo is also IOKit — keep on background thread
                self.getDetailedBatteryInfo(&newInfo)

                DispatchQueue.main.async {
                    if !self.hasBattery { self.hasBattery = true }
                    // Only update if key values actually changed to avoid SwiftUI re-renders
                    if self.batteryInfo.percentage != newInfo.percentage ||
                       self.batteryInfo.isCharging != newInfo.isCharging ||
                       self.batteryInfo.powerWatts != newInfo.powerWatts ||
                       self.batteryInfo.voltage != newInfo.voltage {
