...
class FanControlViewModel: ObservableObject {
    ...
    @Published var thermalHistory: [ThermalSample] = []
    @Published var targetFanSpeed: Int = 0
    ...
    private var lastHighTemperatureNotification: Date = .distantPast
    ...
    private func setupSettingsObservers() {
        ...
        $cpuTemperature
            ...
            .sink { [weak self] temp in
                ...
                self.thermalHistory.append(ThermalSample(date: Date(), temperature: temp, fanSpeed: self.currentFanSpeed))
                if self.thermalHistory.count > 60 { self.thermalHistory.removeFirst(self.thermalHistory.count - 60) }
            }
            .store(in: &cancellables)
    }
    
    private func showHighTempNotification(_ temperature: Double) {
        guard Date().timeIntervalSince(lastHighTemperatureNotification) >= 600 else { return }
        lastHighTemperatureNotification = Date()
        let content = UNMutableNotificationContent()
        content.title = "High Temperature Alert"
        content.subtitle = String(format: "CPU temperature: %.1f°C", temperature)
        content.body = "Consider Smart Auto or check your system."
        content.sound = .default
        UNUserNotificationCenter.current().add(UNNotificationRequest(identifier: "fan.high-temperature", content: content, trigger: nil))
    }
    ...
    private func setupBindings() {
        ...
        fanController.$lastAppliedSpeed
            .receive(on: DispatchQueue.main)
            .assign(to: &$targetFanSpeed)

        fanController.$activePreset
            .receive(on: DispatchQueue.main)
            .assign(to: &$activePreset)

        fanController.$boostEndDate
            .receive(on: DispatchQueue.main)
            .assign(to: &$boostEndDate)
    ...
    func setMonitoringInterval(_ interval: Double) { systemMonitor.setMonitoringInterval(interval) }
    ...
    func applyPreset(_ preset: FanPreset) { fanController.applyPreset(preset) }
    func startBoost() { fanController.startBoost() }
    func stopBoost() { fanController.stopBoost() }
    func setFanSpeed(_ speed: Int, fanIndex: Int) { fanController.setFanSpeed(speed, fanIndex: fanIndex) }
    func setEmergencyProtection(enabled: Bool, temperature: Double) {
        fanController.setEmergencyProtection(enabled: enabled, temperature: temperature)
    }

    func diagnosticsText() -> String {
        let speeds = fanSpeeds.map(String.init).joined(separator: ", ")
        return """
        fan diagnostics
        Generated: \(Date())
        Mode: \(controlMode == .automatic ? "Smart Auto" : "Manual")
        Preset: \(activePreset.rawValue)
        CPU: \(cpuTemperature.map { String(format: "%.1f C", $0) } ?? "Unavailable")
        GPU: \(gpuTemperature.map { String(format: "%.1f C", $0) } ?? "Unavailable")
        Fan speeds: \(speeds.isEmpty ? "Unavailable" : speeds)
        Target RPM: \(targetFanSpeed)
        Fans detected: \(numberOfFans)
        SMC access: \(hasAccess)
        Monitoring: \(isMonitoring)
        Helper installed: \(PermissionsManager.shared.isHelperInstalled)
        Last write succeeded: \(lastWriteSuccess)
    ...
    func exportDiagnostics() {
        ...
        panel.nameFieldStringValue = "fan-diagnostics.txt"
